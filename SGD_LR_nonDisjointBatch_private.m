function [w, cost, e] = SGD_LR_nonDisjointBatch_private(y, X, w0,batch_size, interval_between_errorbar)
...
batch_size = 1;      % change to 5 for KDDCup99 + linear and 10 for MNIST
...
X = data(:,2:end);
 ...
 for t=1:b
        ...
        % calculate eta at time t
          ...
          data = [y, X];
          data_batch = datasample(data,batch_size,1);
          X_batch = data_batch(:,2:end);
          ...
          for i = 1:batch_size
                   ...
                   %gradient at Bt
                   g(i:i,:) = grad(w_t, X_batch(i:i,:), y_batch(i));
          end
